...
class Subject(object):
    ...
    def __add_hemi(self, surf_path, offset=None):
        ...
        if current_hemi_type not in self.hemisphere_type:
        ...
        current_surf_type = self.hemisphere[current_hemi_type].get_surface_type()
    ...
    def add_hemisphere(self, surf_path):
        ...
        if self._is_hemi(hemi_type):
            self.__add_hemi(surf_path)
        else:
            print "Wrong hemisphere!"

    def _del_hemi(self, hemi_type):
    ...
    def _del_surf(self, hemi_type, surf_type):
        """Delete surface data."""
        return self.hemisphere[hemi_type].del_surfs(surf_type)

    def _is_hemi(self, hemi_type):
